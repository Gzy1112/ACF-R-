...
import torch
from model import JZK, xattn_score_i2t1, xattn_score_t2i1, cosine_sim, xattn_score_i2t_val, xattn_score_t2i
...
def encode_data(model, data_loader, log_step=10, logging=print): # <model.JZK object at 0x7fcaf5694a60>
    ...
    for i, (images,  region_feat, targets, lengths, ids,_) in enumerate(data_loader):
    ...
    with torch.no_grad():
        for i, (images,  region_feat,  targets, lengths, ids,_) in enumerate(data_loader):
            ...
            if img_embs is None: #True
                ...
                cap_lens = [0] * len(data_loader.dataset)#5000
            # cache embeddings
            #if region_emb.shape[0] == word_emb.shape[0]:
            img_embs[ids] = img_emb.data.cpu().numpy().copy()
            cap_embs[ids] = cap_emb.data.cpu().numpy()[0:cap_emb.shape[0] // 2,:].copy()
            ...
            word_embs[ids,:word_emb.shape[1],:] = word_emb.data.cpu().numpy()[0:cap_emb.shape[0] // 2,:].copy() #max(lengths) #mismatch
            #word_embs[ids,:word_emb.shape[1],:] = word_emb.data.cpu().numpy().copy() #max(lengths) #mismatch
...
def evalrank(model_path, data_path=None,feature_path=None, split='dev', fold5=False):
    ...
    if not fold5:
        ...
        print("Text to image: %.1f %.1f %.1f %.1f %.1f" %
              mean_metrics[5:10])
        print("rsum: %.1f" % (np.sum(mean_metrics[5:8])+np.sum(mean_metrics[:3])))

    # torch.save({'rt': rt, 'rti': rti}, 'ranks.pth.tar')

def cosine_similarity_np(x1, x2, dim=1, eps=1e-8):
    """Returns cosine similarity between x1 and x2, computed along dim."""
    w12 = x1 @  np.transpose(x2)
    w1 = np.linalg.norm(x1, ord=2)
    w2 = np.linalg.norm(x2, ord=2)
    return np.clip(w12 / (w1 * w2),a_min=eps,a_max=1.1).squeeze()

def shard_xattn_flash(img_embs, cap_embs, region_embs, word_embs, caplens, opt, shard_size=128):
    image_feat = region_embs.mean(1).squeeze()
    cap_feat = word_embs.sum(1).squeeze()
    length = np.array(caplens)
    cap_feat = cap_feat/np.expand_dims(length, 1)
    sims_local = cosine_similarity_np(image_feat, cap_feat, dim=1)
    ...
    with torch.no_grad():
        for i in range(n_im_shard):
            ...
            for j in range(n_cap_shard):
                ...
                wo = Variable(torch.from_numpy(word_embs[cap_start:cap_end])).float().cuda()
                l = caplens[cap_start:cap_end]
                sim_local = xattn_score_i2t_val(re, wo, l, opt)
                sims_local[im_start:im_end, cap_start:cap_end] = sim_local.data.cpu().numpy()
        sys.stdout.write('\n')
...
#              data_path='/data1/data', split="test", fold5=False)
    #/data1/CMIR/new_new_new_jzk_master/run/coco/checkpoint/04111439/checkpoint_14_rsum=501.65999999999997.pth.tar
    start_eval_time = time.time() 
    evalrank('/home/gongziyu/MLASM_Test_F30K/run/f30k/checkpoint/12081106 0.2/checkpoint_19_rsum=560.0197238658777.pth.tar',
                data_path='/data/gongziyu', split="test", fold5=False) #True
    end_eval_time = time.time()
    print('!!!!eval time=', end_eval_time - start_eval_time) 
